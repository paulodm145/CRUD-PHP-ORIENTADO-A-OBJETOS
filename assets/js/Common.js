...
function blockUi(){
    $.blockUI({ message: '<img src="'+ __BASEPATH__ +'assets/css/img/loading.gif" width="20px" /> Aguarde...',
        css: {
            border: 'none',
            padding: '15px',
            backgroundColor: '#000',
...
function validaForm(form) {
    ...
    for (var i = 0; i < form.elements.length; i++) {
        ...
        if (elem.className.search('ckEmail') > -1 && elem.className.search('required') > -1) {
            if (!checkMail(elem.value)) {
                ...
                seg = false;
                applyOnblur = true;
                msgError += "<li>Email Inválido/Incorreto.</li>";
            }
            ...
            if (elem.className.search('required') > -1 && elem.value.length < 1) {
                ...
                seg = false;
                applyOnblur = true;
                msgError += "<li>Preencher Campo: "+String(elem.getAttribute("data-nome")).toUpperCase()+"</li>";
            }
        }
        if(applyOnblur){
    ...
    if(seg){
        ...
        BootstrapDialog.show({
            title: 'Erro ao preencher o formul&aacute;rio. Verifique !',
            message: msgError+"</ul>",
            type:  BootstrapDialog.TYPE_DANGER,
            buttons : [ {label:'Fechar', action:function(dialogItself){dialogItself.close();}} ]
        });
    }
